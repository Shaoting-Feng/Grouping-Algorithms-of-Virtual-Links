...
% 使用的仍然是数组+min代替了优先级队列。待改进
% 在v3的基础上，增加了对于queue的清理。
% 增加了如果不满足jitter约束的返回值。
function [B_min,J,S_MTU,S_BAG,idx] = Find_Minimum_Bandwidth_Configuration_v4(F,N,portB)
    maxQ = 5000; % queue的最大容量
    stopFlag = 0; % 到达最大容量仍然没有找到结果的标志
    ...
    for i = 1:N
        [VL(i).BAG,VL(i).MTU] = Find_Feasible_BAG_MTU_v2(F(i)); 
        if isempty(VL(i).BAG)
            B_min = Inf;
            J = -1;
            S_MTU = [];
    ...
    while 1
        ...
        if length(queue) >= maxQ
            stopFlag = 1;
            maxLevel = max([queue.level]);
            queue = queue([queue.level] >= maxLevel);
            [B_min,index] = min([queue.B]);
        end
        ...
        for k = 1:numel(VL(i).BAG) % 遍历正在设置的VL的所有MTU和BAG组合
            ...
            if B_curr <= B_N && J_curr <= 460 * BB / 8000000 % 满足约束2和约束3则保存
                ...
                % 更改入队的条件，当出现level==N的解之后，把其带宽替代网络端口的B
                if i == N
                    B_N = min(B_curr,B_N);
                end
            end

...
% Initialization_v2.m相较于Initialization.m把Find_Minimum_Bandwidth_Configuration_v2.m改为Find_Minimum_Bandwidth_Configuration_v4.m。
function [F,N] = Initialization_v2(messages)
    ...
    while true
        N = length(F); % 目前的VL数量
        [bestB,~,~,~,VLidx] = Find_Minimum_Bandwidth_Configuration_v4(F,N,messages.B);
    	if bestB ~= Inf
            break;
        end

...
function [B,J,F,BAG,MTU] = Split_forCp(messages,F,N)
...
%         avgL_P = F(x).L_P(1) / 10;
        ...
        if numInVL == 1 % VL_x只剩一条消息考虑的是合并
            ...
            % 第一种情况：保留在VL_x中
            tmpF{1} = F;
            [bestB(1),bestJ(1),MTU{1},BAG{1},~] = Find_Minimum_Bandwidth_Configuration_forCp(tmpF{1},length(tmpF{1}),messages.B);
            ...
            for i = 2:N
                ...
                [bestB(i),bestJ(i),MTU{i},BAG{i},~] = Find_Minimum_Bandwidth_Configuration_forCp(tmpF{i},length(tmpF{i}),messages.B);
            end
